...
#L的DSblock卷积换成depth-wise

#hyper parameter: s=2, N=3, M=2, L=124
...
class DownSamplingBlock_spe(nn.Module):
    
    def __init__(self, in_channel, out_channel):
        super(DownSamplingBlock_spe, self).__init__()
        # self.Conv = nn.Conv2d(in_channel, out_channel, (3,3), 2, 1)
        self.Conv = nn.Conv2d(in_channel, out_channel, (3,3), 2, 1, groups=in_channel)
...
class PGCU(nn.Module):
    ...
    def __init__(self, Channel=31, VecLen=124, NumberBlocks=3):
        ...
        # F.size == (Vec, W, H)
        self.FPConv = nn.Conv2d(3, Channel, (3,3), 1, 1)
        # self.FMConv = nn.Conv2d(Channel, Channel, (3,3), 1, 1)
        self.FMConv = nn.Conv2d(Channel, Channel, (3,3), 1, 1, groups=Channel)
        ...
        for i in range(NumberBlocks):#NumberBlocks = 3
            if i == 0:
                self.GPConv.add_module('DSBlock'+str(i), DownSamplingBlock(3, Channel))
            else:
                self.GPConv.add_module('DSBlock'+str(i), DownSamplingBlock(Channel, Channel))
                self.GMConv.add_module('DSBlock'+str(i-1), DownSamplingBlock_spe(Channel, Channel))
        ...
        for i in range(NumberBlocks):
            if i == 0:
                self.VPConv.add_module('DSBlock'+str(i), DownSamplingBlock(3, Channel))
            else:
                self.VPConv.add_module('DSBlock'+str(i), DownSamplingBlock(Channel, Channel))
                self.VMConv.add_module('DSBlock'+str(i-1), DownSamplingBlock_spe(Channel, Channel))
    ...
    def forward(self, guide, x):
        #x: LrMS / LrHS torch.Size([1, 31, 64, 64])
        #guide: PAN / HrMS torch.Size([1, 3, 64, 64])
