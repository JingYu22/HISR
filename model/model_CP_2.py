...
class PSRTnet(nn.Module):
    def __init__(self, args):
        ...
        init_w(self.w)
        self.CP = DRTLM_v2(rank=4, out_channel=31, height=64, width=64)
    ...
    def forward(self, rgb, lms):
        ...
        self.rgb = rgb
        self.lms = lms
        xt = torch.cat((self.lms, self.rgb), 1)  # Bx34X64x64 #torch.Size([1, 34, 64, 64])
        B, C, H, W = xt.shape
        # xt = self.CP(xt)
        w_out = self.w(H, W, xt)#torch.Size([1, 32, 64, 64])
        # w_out = self.CP(w_out)
        w_out = self.conv(w_out)#[1, 31, 64, 64]
        w_out = self.CP(w_out)
