...
class MainNet(nn.Module):
    ...
    def forward(self, MSI, HSI):
        ################LR-HSI###################
        # UP_LRHSI = F.interpolate(HSI,scale_factor=scale, mode='bicubic') ### (b N h w)
        # UP_LRHSI = UP_LRHSI.clamp_(0,1)
        UP_LRHSI = HSI.clamp_(0,1)
        sz= UP_LRHSI.size(2)
        # if scale == 2:
        #     MSI = F.interpolate(MSI,scale_factor=1/scale, mode='bicubic')
...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=h), qkv)#tuple
