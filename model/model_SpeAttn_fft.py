...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = torch.fft.rfft2(x, norm='ortho') #torch.Size([1, 32, 64, 33])
        # x_imag = x.imag #torch.Size([1, 32, 64, 33])
        # x_real = x.real #torch.Size([1, 32, 64, 33])
        # x = torch.cat([x_real, x_imag], dim=3)#torch.Size([1, 64, 64, 33])
...
class Window_Attention(nn.Module):
    ...
    def __init__(self, dim=32, input_resolution=16, num_heads=8, window_size=4,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, norm='backward'):
        super().__init__()
        self.dim = dim
        ...
        self.dct = DCT2(window_size=64)
        self.idct = IDCT2(window_size=64)
        self.norm = norm

    def forward(self,H, W, x):
        # _, _, H, W = x.shape #torch.Size([1, 32, 64, 64])
        # x = torch.fft.rfft2(x, norm=self.norm) #torch.Size([1, 32, 64, 33])
        # x_imag = x.imag #torch.Size([1, 32, 64, 33])
        # x_real = x.real #torch.Size([1, 32, 64, 33])
        # x = torch.cat([x_real, x_imag], dim=1)#torch.Size([1, 64, 64, 33])

        # x = torch.fft.rfft2(x, dim=(2, 3), norm='ortho')

        x = rearrange(x, 'B C H W -> B (H W) C')#torch.Size([1, 2112, 64])
        B, L, C = x.shape
