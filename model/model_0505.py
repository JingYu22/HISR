...
#block交互+高频refine（高斯）+3rd block改成spe且并行
...
class W2W_Block(nn.Module):
    def __init__(self, img_size=64, in_chans=32, head=8, win_size=4):
        ...
        self.WA4 = Window_Attention(dim=self.in_channels, input_resolution=img_size, num_heads=head,
                                    window_size=win_size)
        self.conv = nn.Conv2d(in_channels=in_chans*4, out_channels=in_chans, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, H, W, x):
        ...
        # window_attention3
        x3 = self.WA3(H, W, x3, mode='spa')

        x4 = self.WA4(H, W, x, mode='spa')

        x_out = shortcut + self.conv(torch.cat((x1,x2,x3,x4),1))
