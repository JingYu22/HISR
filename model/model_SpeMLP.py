...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        # bs,hidden_dim,32x32
        x1 = self.project_in(x1)
        x1 = self.dwconv(x1)
     

        x2 = self.linear1(x)
        # spatial restore
        x2 = rearrange(x2, ' b (h w) (c) -> b c h w ', h = hh, w = hh)
        # bs,hidden_dim,32x32
        x2 = self.dwconv(x2)

        x3 = F.gelu(x2) + F.gelu(x1)
        x3 = rearrange(x3, ' b c h w -> b (h w) c', h = hh, w = hh)          
        x3 = self.linear2(x3)
        return x3
...
class Window_Attention(nn.Module):
    ...
    def __init__(self, dim=32, input_resolution=16, num_heads=8, window_size=4,
        ...
        self.mlp = MLP(dim=dim, hidden_dim=mlp_hidden_dim, bias=False)
