...
#交互(conv+relu)+高频refine（高斯）+3rd block改成spe
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., mode='spe'):
...
class Window_Attention(nn.Module):
    ...
    def forward(self,H, W, x, mode):
        ...
        # attention
        attn_windows = self.attn(x_windows, mode=mode)  # nW*B, window_size*window_size, C
...
class W2W_Block(nn.Module):
    def __init__(self, img_size=64, in_chans=32, head=8, win_size=4):
        ...
        self.WA3 = Window_Attention(dim=self.in_channels, input_resolution=img_size, num_heads=head,
                                    window_size=win_size)
        self.WA4 = Window_Attention(dim=self.in_channels, input_resolution=img_size, num_heads=head,
                                    window_size=win_size)
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels=in_chans*3, out_channels=in_chans, kernel_size=3, stride=1, padding=1),nn.LeakyReLU(0.2, True)
        )

    def forward(self, H, W, x):
        # window_attention1
        shortcut = x
        x1 = self.WA1(H, W, x, mode='spa')

        # shuffle
        x2 = Win_Shuffle(x1, self.win_size)

        # window_attention2
        x2 = self.WA2(H, W, x2, mode='spa')

        # reshuffle
        x3 = Win_Reshuffle(x2, self.win_size)

        # window_attention3
        x3 = self.WA3(H, W, x3, mode='spe')

        # x4 = self.WA4(H, W, x, mode='spa')

        x_out = shortcut + self.conv(torch.cat((x1,x2,x3),1))
