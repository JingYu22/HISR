...
from module.LCB import *
device = "cuda" if torch.cuda.is_available() else "cpu"

#第三个block换成Spe-attn, MLP换成LCB
#加上三个tran block 信息交互
#spa-block及spe-block加上高频作为prior

#0517改动：spe-block 的 decoder 变成 depth-wise
...
class Attention(nn.Module):
    ...
    def forward(self, x, td=None):
        ...
        if td is not None:
            qkv_td = self.qkv(td).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            v = v + qkv_td[2]
...
class Attention_spe(nn.Module):
    ...
    def forward(self, x, td=None):
        ...
        if td is not None:
            td = rearrange(td, ' b (h w) (c) -> b c h w ', h = hh, w = hh) 
            qkv_td = self.qkv_dwconv_C(self.qkv_C(td))
...
# class Decode_Block(nn.Module):#这里可以根据prompt改一下
...
class Decode_Block_dw(nn.Module):
    def __init__(self, inplanes):
        super().__init__()
        self.conv1 = nn.Conv2d(inplanes, inplanes, kernel_size=1, groups=inplanes, bias=False)
        self.conv2 = nn.Conv2d(inplanes, inplanes, kernel_size=1, groups=inplanes, bias=False)
...
class Window_Attention(nn.Module):
    ...
    def __init__(self, dim=32, input_resolution=16, num_heads=8, window_size=4,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, freq_nums=0.25,):
        super().__init__()
        self.dim = dim
    ...
    def forward_features(self,H, W, x, td=None):
        ...
        if td is not None:
            td_windows = window_partition(td, self.window_size)
            td_windows = td_windows.view(-1, self.window_size * self.window_size, C)
            
            # attention
            attn_windows = self.attn(x_windows, td_windows)  # nW*B, window_size*window_size, C
        else:
    ...
    def forward(self, H, W, x):
        input = x
        x = self.forward_features(H, W, input)#torch.Size([1, 32, 64, 64])
        
        prompt = self.fft(x, self.freq_nums)
        cos_sim = F.normalize(x, dim=-1) @ F.normalize(prompt, dim=1)  # B, N, 1
        mask = cos_sim.clamp(0, 1)
        x = x * mask * self.feedback_aug #torch.Size([1, 32, 64, 64])
        td = self.feedback(x)

        x = self.forward_features(H, W, input, td)

        return x

        
#Third block
class Window_Attention_spe(nn.Module):
    r""" PSRT Transformer Block.
    ...
    def __init__(self, dim=32, input_resolution=16, num_heads=8, window_size=4,
        ...
        self.norm2 = norm_layer(dim)
        # mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = nn.Sequential(MBConv(dim, dim, downsample=False, expansion_rate=mlp_ratio, shrinkage_rate = 0.25))
        self.decoder = Decode_Block_dw(dim)
    ...
    def forward_features(self,H, W, x, td=None):
        ...
        attn_windows = attn_windows.view(-1, self.window_size, self.window_size, C)
        x = window_reverse(attn_windows, self.window_size, H, W)  # B H' W' C
        x = x.view(B, H * W, C)#torch.Size([1, 4096, 32])
        # FFN
        # drop = rearrange(self.drop_path(x), 'B H W C -> B C H W')
        # x = shortcut + drop
        x = shortcut + self.drop_path(x)
        norm = rearrange(x, 'B (H W) C -> B C H W', H=H)
        mlp = self.mlp(norm)
        mlp = rearrange(mlp, 'B C H W -> B (H W) C')
...
class W2W_Block(nn.Module):
    def __init__(self, img_size=64, in_chans=32, head=8, win_size=4):
        ...
        self.WA2 = Window_Attention(dim=self.in_channels, input_resolution=img_size, num_heads=head,
                                    window_size=win_size)
        self.WA3 = Window_Attention_spe(dim=self.in_channels, input_resolution=img_size, num_heads=head,
                                    window_size=win_size)
        
        self.conv = nn.Conv2d(self.in_channels*3, self.in_channels, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, H, W, x):
        ...
        # shuffle
        x2 = Win_Shuffle(x1, self.win_size)

        # window_attention2
        x2 = self.WA2(H, W, x2)

        # reshuffle
        x3 = Win_Reshuffle(x2, self.win_size)

        # window_attention3
        x3 = self.WA3(H, W, x3)#([16, 32, 64, 64])

        x_out = torch.cat((x1,x2,x3), 1)#([16, 32, 64, 64])
...
class PSRTnet(nn.Module):
    def __init__(self, args):
        ...
        self.conv_1 = nn.Sequential(
            nn.Conv2d(self.in_channels+3, self.in_channels, 3, 1, 1), nn.LeakyReLU(0.2, True)
         )
    def forward(self, rgb, lms):
        ...
        self.rgb = rgb
        self.lms = lms
        # self.rgb_high = self.high(self.rgb)
        ...
        # refine = torch.cat((self.result,self.rgb_high),1)
        # refine = self.conv_1(refine)
