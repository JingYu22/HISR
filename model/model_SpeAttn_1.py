...
import math
# from others.dct_util import *

#换成spe attn
#第二组window partition方式换成grid
...
def window_partition(x, window_size):
    ...
    windows = rearrange(x, 'b (x w1) (y w2) c -> (b x y) w1 w2 c', w1 = window_size, w2 = window_size)
...
def window_reverse(windows, window_size, H, W):
    ...
    x = rearrange(windows, '(b x y) w1 w2 c -> b (x w1) (y w2) c',x=H, y=W, w1 = window_size, w2 = window_size)
    # x = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(B, H, W, -1)
    return x

def grid_partition(x, window_size):
    """
    Args:
        x: (B, H, W, C)
        window_size (int): window size
    ...
    # B, H, W, C = x.shape
    # x = x.view(B, H // window_size, window_size, W // window_size, window_size, C)
    grids = rearrange(x, 'b (w1 x) (w2 y) c -> (b x y) w1 w2 c', w1 = window_size, w2 = window_size)
    # windows = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, window_size, window_size, C)
    return grids

def grid_reverse(grids, window_size, H, W):
    """
    Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): Window size
    ...
    # x = windows.view(B, H // window_size, W // window_size, window_size, window_size, -1)
    # x = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(B, H, W, -1)
    x = rearrange(grids, '(b x y) w1 w2 c -> b (w1 x) (w2 y) c',x=H, y=W, w1 = window_size, w2 = window_size)
    return x
...
class Grid_Attention(nn.Module):
    ...
    def forward(self,H, W, x):
        ...
        x = self.norm1(x)
        x = x.view(B, H, W, C)
        x_windows = grid_partition(x, self.window_size)  # nW*B, window_size, window_size, C
        ...
        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size, self.window_size, C)
        x = grid_reverse(attn_windows, self.window_size, H, W)  # B H' W' C
        x = x.view(B, H * W, C)
        # FFN
        x = shortcut + self.drop_path(x)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
...
class G2G_Block(nn.Module):
    def __init__(self, img_size=64, in_chans=32, head=8, win_size=4):
        ...
        self.in_channels = in_chans
        self.win_size = win_size
        self.GA1 = Grid_Attention(dim=self.in_channels, input_resolution=img_size, num_heads=head,
                                    window_size=win_size)
        self.GA2 = Grid_Attention(dim=self.in_channels, input_resolution=img_size, num_heads=head,
                                    window_size=win_size)
        self.GA3 = Grid_Attention(dim=self.in_channels, input_resolution=img_size, num_heads=head,
                                    window_size=win_size)
...
class Pyramid_Block(nn.Module):
    def __init__(self, num=3, img_size=64, in_chans=32, head=8, win_size=8):
        ...
        for i_layer in range(self.num_layers):
            layer1 = W2W_Block(img_size=img_size, in_chans=in_chans, head=head, win_size=win_size//(2**i_layer))
            layer2 = G2G_Block(img_size=img_size, in_chans=in_chans, head=head, win_size=win_size//(2**i_layer))
...
class PSRTnet(nn.Module):
    def __init__(self, args):
        ...
        self.w = Block(out_num=1, inside_num=3, img_size=self.img_size, in_chans=34, embed_dim=self.embed, head=8,
                       win_size=8)
